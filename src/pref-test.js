...
const performanceTest = (fn, iterations = 100000) => {
  ...
  const originalValues = new Array(iterations).fill(0).map(() => cloneDeep(newUser));
  const start = performance.now();
  originalValues.forEach((user) => fn(user));
  return (performance.now() - start) / iterations;
};

console.log(`reference compare = ${(performanceTest(addAndCompareReference) * 1000).toFixed(3)}μs`);

console.log(`deepcopy compare = ${(performanceTest(addAndCompareDeepcopy) * 1000).toFixed(3)}μs`);

// node pref-test.js
// output
// reference compare = 1.073μs
// deepcopy compare = 7.516μs
